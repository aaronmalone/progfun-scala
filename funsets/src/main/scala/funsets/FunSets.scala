...
object FunSets {
  ...
  private def not[A](p: A => Boolean): (A => Boolean) = !p(_)
  ...
  /**
    ...
    */
  def filter(s: Set, p: Int => Boolean): Set = elem => p(elem) && contains(s, elem)
  ...
  def forall(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      if (a > bound) true
      else if (contains(s, a) && !p(a)) false //TODO MAYBE REFACTOR
      else iter(a + 1)
    }
    iter(-bound)
  }
